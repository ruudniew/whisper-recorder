...
class AudioProcessor:
    ...
    def __init__(self, **kwargs):
        ...
        # Check if we're receiving raw PCM (for our custom client)
        self.raw_pcm_mode = getattr(models.args, 'raw_pcm', False)
        ...
        # Silence tracking for buffer management
        self.last_meaningful_transcription_time = None
        self.silence_timeout_seconds = 2.0  # Clear buffer after 2 seconds of silence
        self.last_buffer_content = ""
        
        # Models and processing
        self.asr = models.asr
        self.tokenizer = models.tokenizer
        self.diarization = models.diarization
        self.ffmpeg_process = None if self.raw_pcm_mode else self.start_ffmpeg_decoder()
    ...
    async def process_raw_pcm(self, pcm_data):
        """Process raw PCM data directly without FFmpeg decoding."""
        # Add to PCM buffer
        ...
        while len(self.pcm_buffer) >= self.bytes_per_sec:
            ...
            pcm_array = self.convert_pcm_to_float(bytes(chunk))
    ...
    async def transcription_processor(self):
        ...
        while True:
            try:
                ...
                # Additional check: prevent repeating last word during silence
                current_time = time()
                
                # Check for silence timeout - clear buffer if we've been silent too long
                if self.last_meaningful_transcription_time:
                    time_since_last = current_time - self.last_meaningful_transcription_time
                    if time_since_last > self.silence_timeout_seconds:
                        logger.debug(f"Silence detected for {time_since_last:.1f}s, clearing buffer")
                        self.buffer_transcription = ""
                        self.last_buffer_content = ""
                        self.last_meaningful_transcription_time = None
                
                if buffer_text and self.buffer_transcription:
                    # Check if the buffer is just repeating the last word(s)
                    last_words = self.buffer_transcription.strip().split()[-2:]  # Get last 2 words
                    buffer_words = buffer_text.strip().split()
                    
                    # If buffer only contains repetition of last word(s), clear it
                    if last_words and buffer_words and len(set(buffer_words)) == 1:
                        if buffer_words[0] in last_words:
                            logger.debug(f"Detected repeated word '{buffer_words[0]}' in buffer, clearing")
                            buffer_text = ""
                
                # Check if this is just a repetition of the same content
                if buffer_text and buffer_text == self.last_buffer_content:
                    logger.debug("Ignoring repeated buffer content")
                    buffer_text = ""
                
                # Update tracking if we have meaningful content
                if buffer_text and buffer_text.strip():
                    self.last_meaningful_transcription_time = current_time
                    self.last_buffer_content = buffer_text
    ...
    async def create_tasks(self):
        ...
        # Only create FFmpeg reader task if not in raw PCM mode
        if not self.raw_pcm_mode:
    ...
    async def cleanup(self):
        ...
        # Only handle FFmpeg cleanup if not in raw PCM mode
        if not self.raw_pcm_mode and self.ffmpeg_process:
    ...
    async def process_audio(self, message):
        ...
        # If already stopping, ignore further audio
        if self.is_stopping:
            logger.warning(f"AudioProcessor is stopping. Ignoring incoming audio message (length: {len(message)}).")
            return
            
        # Handle raw PCM mode differently
        if self.raw_pcm_mode:
            if not message:  # Empty message signals stop
                logger.info("Empty audio message received, initiating stop sequence.")
                self.is_stopping = True
                return
                
            # Process raw PCM directly
            await self.process_raw_pcm(message)
            return
        
        # Original WebM processing code
        if self.ffmpeg_process and self.ffmpeg_process.stdin and self.ffmpeg_process.stdin.closed:
            logger.warning(f"FFmpeg stdin is closed. Ignoring incoming audio message (length: {len(message)}).")
            return
